/**
ui.control.js
This code will control the primary functions of the UI in the app
**/
...
//        var windowWidth = $(window).width();
    ...
    updateViz:function() {
        ...
        for(var i = 0; i < exportBtns.length; i++) {
            ...
            var routeTypeKey = btn.attr('class');
            var routeName = reverserouteLookup[routeTypeKey];

            if(btn.find('.inactive').length == 0) {
                exportArray.push(routeName);
                selectionData.exportCategories[routeName] = true;
            } else {
                selectionData.exportCategories[routeName] = false;
            }
        }
        //imports esecond
        var importArray = []
        ...
        for(var i = 0; i < importBtns.length; i++) {
            ...
            var routeName = reverserouteLookup[routeTypeKey];
            if(btn.find('.inactive').length == 0) {
                importArray.push(routeName);
                selectionData.importCategories[routeName] = true;
            } else {
                selectionData.importCategories[routeName] = false;
            }
        }
...
*/
    ...
    drawBarGraph: function() {
        ...
        for(var type in reverserouteLookup) {
        ...
        for(var i = 0; i < importLabelArray.length; i++) {
            ...
            if(pieceHeight < smallLabelSize) {
                ...
                var textLabel = importLabel.append('text').text(function(d) {
                    return reverserouteLookup[d.type].split(' ')[0].toUpperCase();
                ...
                var routeLabel  =importLabel.append('text').text('ROUTES').attr('text-anchor','end').attr('y',21)
                ...
                var numericLabelEle = numericLabel[0][0];
                var textLabelEle = textLabel[0][0];
                var routeLabelEle = routeLabel[0][0];
                labelWidth = numericLabelEle.getComputedTextLength() > textLabelEle.getComputedTextLength() ? numericLabelEle.getComputedTextLength() : textLabelEle.getComputedTextLength();
                if(routeLabelEle.getComputedTextLength() > labelWidth) {
                    labelWidth = routeLabelEle.getComputedTextLength();
                }
            }
        ...
        for(var i = 0; i < exportLabelArray.length; i++) {
            ...
            if(pieceHeight < smallLabelSize) {
                ...
                var routeLabel  =exportLabel.append('text').text('ROUTES').attr('text-anchor','start').attr('y',21)
        ...
        importLabel.enter().append('text').attr('x',midX).attr('text-anchor','end').text('INBOUND')
        ...
        exportLabel.enter().append('text').attr('x',midX+10).text('OUTBOUND')
