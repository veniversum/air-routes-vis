...
function attachMarkerToCountry( countryName, importance ){
	...
	marker.update = function(){
		...
		var importanceScale = this.importance/selectedCountry.summary.total * 200 ;
		...
		this.setSize( s ); 
		 ...
		 /*if( this.selected )
			...
			this.setVisible( ( abspos.z > 60 ) && s > 8 );	

		var zIndex = Math.floor( 1000 - abspos.z + 5 );
	...
	var tiny = (importance < selectedCountry.summary.total/10000) && (!marker.selected);	
	marker.tiny = tiny;
	...
	if(country.exportedAmount > 0 && country.importedAmount > 0) {
	   importExportText += "Inbound Routes:" + numberWithCommas(country.importedAmount) + "<br />" +
	       "Outbound Routes:"+numberWithCommas(country.exportedAmount);
	} else if(country.exportedAmount > 0 && country.importedAmount == 0) {
	   importExportText += "Outbound Routes:"+numberWithCommas(country.exportedAmount)+"<br />&nbsp;";
	} else if(country.exportedAmount == 0 && country.importedAmount > 0) {
	   importExportText += "Inbound Routes:"+numberWithCommas(country.importedAmount)+"<br />&nbsp;";
	}
