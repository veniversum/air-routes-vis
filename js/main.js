...
//	contains the data loaded from the flights data file
//	contains a list of years, followed by trades within that year
//	properties for each "trade" is: e - exporter, i - importer, v - value (USD), wc - route type code (see table)
var timeBins;
...
var reverserouteLookup = new Object();
for( var i in routeLookup ){
	var name = i;
	var code = routeLookup[i];
	reverserouteLookup[code] = name;
}	    	
...
// 			specular: 	0x333333,
	...
	selectVisualization( timeBins, '2010', ['UNITED STATES'], ['Passenger Traffic','Cargo Traffic', 'Movements'], ['Passenger Traffic','Cargo Traffic', 'Movements'] );					
...
function getHistoricalData( country ){
	...
	for( var i in timeBins ){
		...
		for( var s in yearBin ){
			var set = yearBin[s];
			var categoryName = reverserouteLookup[set.wc];
